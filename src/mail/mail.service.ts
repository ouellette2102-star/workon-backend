...
export class MailService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (apiKey) {
      try {
        ...
        this.logger.warn('⚠️ Failed to initialize Resend: ' + (error as Error).message);
      }
    } else {
      this.logger.warn(
  ...
  async sendEmail(options: SendEmailOptions): Promise<{ success: boolean; messageId?: string; error?: string }> {
    ...
    if (!this.resend) {
      ...
      if (this.isDev) {
        // In dev, log more details for debugging
        this.logger.debug(`[Mail] HTML preview: ${html.substring(0, 500)}...`);
      }
